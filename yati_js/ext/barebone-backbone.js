...
barebone.QueryParams = Backbone.Model.extend({
    ...
    getForQuery: function (parent, options) {
        options || (options = {});
        if (parent) return _(this.attributes).pick('serializer');
        return _(this.attributes).omit(['count'].concat(options.nopagination ? ['page', 'pageSize'] : []));
    },
    getPages: function () {
...
barebone.sync_api = function (method, model, options) {
    ...
    if (model.queryParams) { _(finalParams).extend(model.queryParams.getForQuery(false, {nopagination:isModel})); }
    ...
    if (options.url.indexOf('?') === -1 && options.url.substr(-1) != '/') options.url += '/';
